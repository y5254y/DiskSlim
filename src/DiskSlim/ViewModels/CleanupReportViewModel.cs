...
public partial class CleanupReportViewModel : ObservableObject
{
    ...
    [ObservableProperty]
    private string _statusMessage = Localizer.Get("Vm.CleanupReport.Loading", "加载历史记录中...");
    ...
    public async Task LoadReportsAsync()
    {
        ...
        try
        {
            ...
            StatusMessage = HasReports
                ? Localizer.Format("Vm.CleanupReport.Count", "共 {0} 条清理记录", Reports.Count)
                : Localizer.Get("Vm.CleanupReport.Empty", "暂无清理记录");
            if (HasReports && SelectedReport == null)
                SelectedReport = Reports[0];
        }
        catch (Exception ex)
        {
            StatusMessage = Localizer.Format("Vm.CleanupReport.LoadFailed", "加载失败：{0}", ex.Message);
        }
        finally
        {
            IsLoading = false;
    ...
    private async Task DeleteSelectedReportAsync()
    {
        ...
        {
            StatusMessage = Localizer.Format("Vm.CleanupReport.DeleteFailed", "删除失败：{0}", ex.Message);
        }
    }
    ...
    private async Task ExportToTextAsync()
    {
        ...
        try
        {
            ...
            await SaveFileAsync(content, Localizer.Get("Vm.CleanupReport.ExportTxtName", "DiskSlim_清理报告.txt"), "txt");
        }
        catch (Exception ex)
        {
            StatusMessage = Localizer.Format("Vm.CleanupReport.ExportFailed", "导出失败：{0}", ex.Message);
        }
    }
    ...
    private async Task ExportToHtmlAsync()
    {
        ...
        try
        {
            ...
            await SaveFileAsync(content, Localizer.Get("Vm.CleanupReport.ExportHtmlName", "DiskSlim_清理报告.html"), "html");
        }
        catch (Exception ex)
        {
            StatusMessage = Localizer.Format("Vm.CleanupReport.ExportFailed", "导出失败：{0}", ex.Message);
    ...
    private async Task ExportToCsvAsync()
    {
        ...
        try
        {
            ...
            await SaveFileAsync(content, Localizer.Get("Vm.CleanupReport.ExportCsvName", "DiskSlim_清理报告.csv"), "csv");
        }
        catch (Exception ex)
        {
            StatusMessage = Localizer.Format("Vm.CleanupReport.ExportFailed", "导出失败：{0}", ex.Message);
    ...
    private static async Task SaveFileAsync(string content, string suggestedName, string extension)
    {
        ...
        if (extension == "txt")
            picker.FileTypeChoices.Add(Localizer.Get("Vm.CleanupReport.TxtType", "文本文件"), new List<string> { ".txt" });
        else if (extension == "csv")
            picker.FileTypeChoices.Add(Localizer.Get("Vm.CleanupReport.CsvType", "CSV 文件"), new List<string> { ".csv" });
        else
            picker.FileTypeChoices.Add(Localizer.Get("Vm.CleanupReport.HtmlType", "HTML 文件"), new List<string> { ".html" });
